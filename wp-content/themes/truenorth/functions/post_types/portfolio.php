...
function ci_update_cpt_portfolio_meta( $post_id ) {
	...
	if ( ! ci_can_save_meta( 'cpt_portfolio' ) ) {
		return;
	}
	...
	update_post_meta( $post_id, 'portfolio_template', in_array( $_POST['portfolio_template'], array( 'list', 'slideshow' ), true ) ? $_POST['portfolio_template'] : 'slideshow' );
...
function ci_add_cpt_portfolio_meta_box( $object, $box ) {
	...
	?><div class="ci-cf-wrap"><?php
		...
		ci_metabox_open_tab( __( 'Details', 'truenorth' ) );
			ci_metabox_guide( __( 'Select <em>Add Field</em> as many times as you want to create a list of info details. You can delete a row by clicking on its <em><span class="dashicons dashicons-no"></span> Remove Me</em> button. You may also click and drag the fields to re-arrange them.', 'truenorth' ) );
			?>
			<fieldset class="ci-repeating-fields">
				<div class="inner">
						...
						if ( ! empty( $fields ) ) {
							foreach ( $fields as $field ) {
								...
								<div class="post-field">
									<label><?php esc_html_e( 'Title:', 'truenorth' ); ?> <input type="text" name="ci_repeatable_detail_title[]" value="<?php echo esc_attr( $field['title'] ); ?>" class="widefat" /></label>
									<label><?php esc_html_e( 'Description:', 'truenorth' ); ?> <input type="text" name="ci_repeatable_detail_description[]" value="<?php echo esc_attr( $field['description'] ); ?>" class="widefat" /></label>
					...
					<div class="post-field field-prototype" style="display: none;">
						<label><?php esc_html_e( 'Title:', 'truenorth' ); ?> <input type="text" name="ci_repeatable_detail_title[]" value="" class="widefat" /></label>
						<label><?php esc_html_e( 'Description:', 'truenorth' ); ?> <input type="text" name="ci_repeatable_detail_description[]" value="" class="widefat" /></label>
						<p class="ci-repeating-remove-action"><a href="#" class="button ci-repeating-remove-field"><i class="dashicons dashicons-dismiss"></i><?php esc_html_e( 'Remove me', 'truenorth' ); ?></a></p>
					</div>
				</div>
				<a href="#" class="ci-repeating-add-field button"><i class="dashicons dashicons-plus-alt"></i><?php esc_html_e( 'Add Field', 'truenorth' ); ?></a>
			</fieldset>
			<?php
		ci_metabox_close_tab();
		...
		ci_metabox_open_tab( __( 'Video', 'truenorth' ) );
			...
			ci_metabox_guide( wp_kses( sprintf( __( 'Just paste the URL of your video here. <a href="%s">WordPress supports a wide range of video services.</a>', 'truenorth' ), 'https://codex.wordpress.org/Embeds' ), truenorth_get_allowed_tags( 'guide' ) ) );
