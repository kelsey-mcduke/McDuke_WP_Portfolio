...
<?php while ( have_posts() ) : the_post(); ?>
	...
	<article id="entry-<?php the_ID(); ?>" <?php post_class( 'entry' ); ?>>
				...
				<div class="ci-slider portfolio-slider loading">
					<ul class="slides">
						<?php while ( $gal->have_posts() ) : $gal->the_post(); ?>
							...
							<li>
								<a class="ci-lightbox" href="<?php echo esc_url( wp_get_attachment_image_url( get_the_ID(), 'large' ) ); ?>" title="<?php echo esc_attr( $img_info['caption'] ); ?>">
									<img src="<?php echo esc_url( wp_get_attachment_image_url( get_the_ID(), 'post-thumbnail' ) ); ?>" alt="<?php echo esc_attr( $img_info['alt'] ); ?>" />
								</a>
							</li>
						<?php endwhile; ?>
		...
		<?php elseif ( has_post_thumbnail() ) : ?>
			<figure class="entry-featured">
				<a href="<?php echo esc_url( get_the_post_thumbnail_url( get_the_ID(), 'large' ) ); ?>" class="ci-lightbox">
		...
		<div class="row">
			...
			<div class="col-md-3">
				<dl class="entry-meta group">
					<dt><?php esc_html_e( 'Date:', 'truenorth' ); ?></dt>
					...
					<?php foreach ( $details as $detail ) : ?>
						...
						<dd><?php echo wp_kses_post( make_clickable( $detail['description'] ) ); ?></dd>
					<?php endforeach; ?>

					<?php if ( get_the_term_list( get_the_ID(), 'portfolio_category' ) !== '' ) : ?>
						<dt><?php esc_html_e( 'Categories:', 'truenorth' ); ?></dt>
