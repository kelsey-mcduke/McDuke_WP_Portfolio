...
function ci_dashboard_add_widgets() {
	if ( apply_filters( 'ci_show_dashboard_rss', true ) && // Should not be shown if the filter evaluates to false
		...
		wp_add_dashboard_widget( 'ci_dashboard_widget_igniter_news', __( 'CSSIgniter News', 'truenorth' ), 'ci_dashboard_widget_igniter_news' );
	}
}
endif;
...
function ci_dashboard_widget_igniter_news() {
	$feeds = array(
		'news' => array(
			...
			'title'        => __( 'CSSIgniter Blog', 'truenorth' ),
			'items'        => 3,
			'show_summary' => 1,
			'show_author'  => 0,
			'show_date'    => 1,
