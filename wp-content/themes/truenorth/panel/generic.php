...
function ci_generator_tag( $gen, $type ) {
	if ( apply_filters( 'ci_show_generator_tag', true ) && // Should not be shown if the filter evaluates to false
		...
		if ( 'html' === $type ) {
			$gen .= PHP_EOL . '<meta name="generator" content="' . esc_attr( 'CSSIgniter - ' . TRUENORTH_NICENAME ) . '">';
		} elseif ( 'xhtml' === $type ) {
			$gen .= PHP_EOL . '<meta name="generator" content="' . esc_attr( 'CSSIgniter - ' . TRUENORTH_NICENAME ) . '" />';
		}
	}
...
function ci_human_time_diff( $from, $to = '' ) {
	...
	if ( $diff < MINUTE_IN_SECONDS ) {
		$since = __( 'Less than a minute ago', 'truenorth' );
		...
		$since = sprintf( _n( '%s minute ago', '%s minutes ago', $mins, 'truenorth' ), $mins );
		...
		$since = sprintf( _n( '%s hour ago', '%s hours ago', $hours, 'truenorth' ), $hours );
		...
		$since = sprintf( _n( '%s day ago', '%s days ago', $days, 'truenorth' ), $days );
		...
		$since = sprintf( _n( '%s month ago', '%s months ago', $months, 'truenorth' ), $months );
	}
...
function get_child_or_parent_file_uri( $path ) {
	_deprecated_function( __FUNCTION__, '2019-02-26', 'get_theme_file_uri()' );
...
function ci_column_classes( $cols_number, $parent_cols = 16, $reset = false ) {
	...
	if ( true === $parent_cols || false === $parent_cols ) {
		$reset       = $parent_cols;
		$parent_cols = 16;
	}
	...
	if ( is_integer( $parent_cols ) && ! is_string( $parent_cols ) ) {
		...
		if ( 3 === intval( $cols_number ) && 16 === intval( $parent_cols ) ) {
			$classes[] = 'one-third';
		} else {
		...
		if ( ! in_array( $parent_cols, $combinations, true ) ) {
			return '';
		}
...
if ( ! function_exists( 'truenorth_classes' ) ) :
/**
 ...
 * @return array
 */
function truenorth_classes() {
	$version = str_replace( '.', '-', TRUENORTH_VERSION );

	$classes['theme']         = 'ci-' . TRUENORTH_NAME;
	$classes['theme_version'] = 'ci-' . TRUENORTH_NAME . '-' . $version;
...
function ci_list_cat_tag_tax( $separator = ', ' ) {
	...
	if ( ! empty( $the_terms_list ) ) {
		...
		return __( 'Uncategorized', 'truenorth' );
	}
}
endif;
...
function ci_pagination( $args = array(), $query = false ) {
	...
	$defaults = apply_filters( 'ci_pagination_default_args', array(
		...
		'prev_text'             => __( 'Older posts', 'truenorth' ),
		'next_text'             => __( 'Newer posts', 'truenorth' ),
	...
	if ( 'paginate_links' === $method ) {
		...
		if ( ( $pagenavi_exists && 'wp_pagenavi' === $method ) ||
			( $pagenavi_exists && empty( $method ) ) ) {
		...
		>
			<?php
				switch ( $use_pagination ) {
					...
					default:
						?>
						<div <?php echo( empty( $args['prev_link_class'] ) ? '' : 'class="' . esc_attr( $args['prev_link_class'] ) . '"' ); ?>><?php next_posts_link( '<span class="nav-prev-symbol nav-symbol">&laquo;</span> ' . $args['prev_text'], $query->max_num_pages ); ?></div>
						<div <?php echo( empty( $args['next_link_class'] ) ? '' : 'class="' . esc_attr( $args['next_link_class'] ) . '"' ); ?>><?php previous_posts_link( $args['next_text'] . ' <span class="nav-next-symbol nav-symbol">&raquo;</span>' ); ?></div>
						<?php
						break;
				}
			?>
...
function ci_setting( $setting ) {
	...
	if ( isset( $ci[ $setting ] ) && ( ! empty( $ci[ $setting ] ) ) ) {
		$value = $ci[ $setting ];
	}
...
function ci_logo( $before = '', $after = '' ) {
	...
	$snippet .= '<a href="' . esc_url( home_url( '/' ) ) . '">';

	if ( ci_setting( 'logo' ) ) {
		$retina_attribute = '';
		if ( ci_setting( 'logox2' ) ) {
			$retina_attribute = sprintf( 'data-rjs="%s"', esc_url( ci_setting( 'logox2' ) ) );
		}

		$snippet .= sprintf( '<img src="%s" alt="%s" %s />',
			esc_url( ci_setting( 'logo' ) ),
...
function has_readmore() {
	if ( false === strpos( get_the_content(), '#more-' ) ) {
		return false;
	} else {
		return true;
	}
 ...
 * @return string|int Empty string on zero, or a positive integer.
 */
...
function wp_dropdown_posts( $args = '', $name = 'post_id' ) {
	...
	if ( ! empty( $pages ) || true === (bool) $select_even_if_empty ) {
 ...
 */
function truenorth_get_allowed_tags( $context = '' ) {
	$allowed = array(
		'a'       => array(
			'href'   => true,
			'title'  => true,
	...
	return apply_filters( 'truenorth_get_allowed_tags', $allowed, $context );
}


if ( ! function_exists( 'truenorth_update_check' ) ) :
function truenorth_update_check() {
	if ( TRUENORTH_UPDATES === false ) {
		return false;
	}

	$versions_url        = apply_filters( 'truenorth_update_url', 'http://www.cssigniter.com/theme_versions.json' );
	$update_period       = apply_filters( 'truenorth_update_period', 3 * 24 * 60 * 60 );
	$error_update_period = apply_filters( 'truenorth_update_period_after_error', 8 * 60 * 60 );
	$transient_name      = TRUENORTH_NAME . '_latest_version';
	$themes_versions     = '';
	...
	if ( false === $latest_version ) {
		...
		if ( is_null( $themes_versions ) || false === $themes_versions ) {
			set_transient( $transient_name, - 1, $error_update_period );

			return false;
		}

		if ( ! isset( $themes_versions[ TRUENORTH_NAME ] ) ) {
			set_transient( $transient_name, - 1, $error_update_period );

			return false;
		}

		$latest_version = $themes_versions[ TRUENORTH_NAME ];
...
if ( ! function_exists( 'truenorth_update_check_admin_handler' ) ) :
/**
 ...
 * @return object
 */
function truenorth_update_check_admin_handler( $transient ) {
	if ( TRUENORTH_UPDATES === true ) {
		$latest_version = truenorth_update_check();
		if ( ( false !== $latest_version ) && version_compare( $latest_version, TRUENORTH_VERSION, '>' ) ) {
			$theme      = wp_get_theme();
			$theme_slug = $theme->get_template();

			$transient->checked[ TRUENORTH_NAME ] = TRUENORTH_VERSION;
			$transient->response[ $theme_slug ]  = array(
				'theme'       => $theme_slug,
				'new_version' => $latest_version,
				'url'         => "https://www.cssigniter.com/changelog/{$theme_slug}",
				'package'     => false,
			);
		}
	}
...
//
if ( ! function_exists( 'add_truenorth_support' ) ) :
function add_truenorth_support( $feature, $options = null ) {
	global $_truenorth_features;

	if ( is_null( $options ) ) {
		$_truenorth_features[ $feature ] = true;
	} elseif ( ! is_null( $options ) && is_array( $options ) ) {
		$_truenorth_features[ $feature ] = $options;
	} else {
		trigger_error( 'Argument 2 of add_truenorth_support() should be an array.', E_USER_NOTICE );
	}
}
endif;

if ( ! function_exists( 'get_truenorth_support' ) ) :
function get_truenorth_support( $feature ) {
	global $_truenorth_features;
	if ( ! isset( $_truenorth_features[ $feature ] ) ) {
		return false;
	} else {
		return $_truenorth_features[ $feature ];
	}
}
endif;

if ( ! function_exists( 'remove_truenorth_support' ) ) :
function remove_truenorth_support( $feature ) {
	global $_truenorth_features;

	if ( ! isset( $_truenorth_features[ $feature ] ) ) {
		return false;
	}
	unset( $_truenorth_features[ $feature ] );
 ...
 */
function truenorth_asset_version( $version = false ) {
 ...
 * @return void|string
 */
function truenorth_main_menu_fallback( $args ) {
	$id    = ! empty( $args['menu_id'] ) ? $args['menu_id'] : 'menu-' . $args['theme_location'];
	$class = $args['menu_class'];

	$items = apply_filters( 'truenorth_main_menu_fallback_items', array(
		home_url( '/' )              => __( 'Home', 'truenorth' ),
		admin_url( 'nav-menus.php' ) => __( 'Set primary menu', 'truenorth' ),
	) );
...
function ci_enqueue_modernizr() {
	wp_enqueue_script( 'modernizr', get_template_directory_uri() . '/panel/scripts/modernizr-2.6.2.js', array(), truenorth_asset_version( '2.6.2' ), false );
}
endif;
...
function ci_print_selectivizr() {
	...
	<!--[if (gte IE 6)&(lte IE 8)]>
		<script type="text/javascript" src="<?php echo esc_url( get_template_directory_uri() . '/panel/scripts/selectivizr-min.js' ); ?>"></script>
	<![endif]-->
	<?php
}
endif;
