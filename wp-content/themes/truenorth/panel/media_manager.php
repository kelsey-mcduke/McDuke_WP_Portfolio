...
function ci_make_custom_image_sizes_selectable( $image_sizes ) {
	...
	foreach ( $additional_image_sizes as $id => $data ) {
		if ( ! isset( $image_sizes[ $id ] ) ) {
			...
			// and unset their ids.
			if ( in_array( $id, array( 'ci_featured_single', 'ci_featured_full' ), true ) ) {
				continue;
			}

			// Ignore sizes that we don't care about.
			if ( ! in_array( $id, array( 'post-thumbnail' ), true ) && ! in_array( substr_left( $id, 3 ), array( 'ci_', 'ci-' ), true ) ) {
				continue;
			}

			// Let's create a readable name.
			$text = $id;
			$text = str_replace( array( 'ci_', 'ci-', 'featgal', TRUENORTH_NAME ), '', $text );
...
function ci_enqueue_media_manager_scripts() {
	...
	if ( ! wp_script_is( 'ci-media-manager-3-5', 'enqueued' ) ) {
		$settings = array(
			...
			'tUseThisFile'        => esc_html__( 'Use this file', 'truenorth' ),
			'tUseTheseFiles'      => esc_html__( 'Use these files', 'truenorth' ),
			'tUpdateGallery'      => esc_html__( 'Update gallery', 'truenorth' ),
			'tLoading'            => esc_html__( 'Loading...', 'truenorth' ),
			'tPreviewUnavailable' => esc_html__( 'Gallery preview not available.', 'truenorth' ),
			'tRemoveFromGallery'  => esc_html__( 'Remove from gallery', 'truenorth' ),
			'tSelectFile'         => esc_html__( 'Select file', 'truenorth' ),
			'tSelectFiles'        => esc_html__( 'Select files', 'truenorth' ),
			'tRemoveImage'        => esc_html__( 'Remove image', 'truenorth' ),
		);
...
function ci_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	...
	<div class="ci-media-manager-gallery">
		<input type="button" class="ci-upload-to-gallery button" value="<?php esc_attr_e( 'Add Images', 'truenorth' ); ?>"/>
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php esc_html_e( 'Randomize order', 'truenorth' ); ?></label></p>
				...
				if ( false !== $images && is_array( $images ) ) {
					foreach ( $images as $image ) {
						...
						<div class="thumb">
							...
							<a href="#" class="close media-modal-icon" title="<?php echo esc_attr( __( 'Remove from gallery', 'truenorth' ) ); ?>"></a>
						</div>
						<?php
					}
				}
			?>
			<p class="ci-upload-to-gallery-preview-text"><?php esc_html_e( 'Your gallery images will appear here', 'truenorth' ); ?></p>
		</div>
	</div>
	<?php
}
...
function ci_featgal_get_images( $csv = false ) {
	...
	if ( count( $ids ) > 0 ) {
		...
		foreach ( $ids as $id ) {
			$thumb_file = wp_get_attachment_image_url( $id, 'ci_featgal_small_thumb' );
