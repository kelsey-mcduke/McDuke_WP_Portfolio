...
function ci_term_meta_init() {
	...
	foreach ( $taxonomies as $taxonomy ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( false === $meta || ( is_array( $meta ) && 0 === count( $meta ) ) ) {
				continue;
			}
...
function ci_term_meta_fields() {
	 ...
	 *      'default' => 'value',
	 *      'add_form_callback' => 'truenorth_term_meta_add',
	 *      'edit_form_callback' => 'truenorth_term_meta_edit',
	 *      'validate_callback' => 'truenorth_term_meta_validate_layout',
	 *      'taxonomies' => array(
	 *          'category',
	 *          'post_tag',
	 *      ),
...
function ci_term_meta_add_form_fields( $taxonomy ) {
	...
	foreach ( $fields as $k => $v ) {
		if ( in_array( $taxonomy, $v['taxonomies'], true ) && is_callable( $v['add_form_callback'], true, $callable ) ) {
...
function ci_term_meta_edit_form_fields( $term, $taxonomy ) {
	$fields = ci_term_meta_fields();
	foreach ( $fields as $k => $v ) {
		if ( in_array( $taxonomy, $v['taxonomies'], true ) && is_callable( $v['edit_form_callback'], true, $callable ) ) {
...
function ci_term_meta_create_form( $term_id, $tt_id, $taxonomy ) {
	$fields = ci_term_meta_fields();
	foreach ( $fields as $k => $v ) {
		if ( in_array( $taxonomy, $v['taxonomies'], true ) && isset( $_POST[ $v['form_name'] ] ) ) {
			$value = $v['default'];
